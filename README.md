...
- KIS·LS·신한 등 증권사 어댑터는 최신 공식 문서를 확인하고 모의환경에서 검증하기 전까지 실패하도록 유지
...
플러그인 자체에는 검증되지 않은 실주문 API를 넣지 않습니다. 실시간 데이터, 주문 중복 방지, 잔고·미체결·체결 대사와 증권사별 paper/live 계약이 완성되지 않은 코드는 실거래 준비가 된 것이 아닙니다.
